...
session.get('http://pan.baidu.com', headers=headers,verify=False)
...
def get_token(gid, callback):
    ...
    resp = session.get(url='https://passport.baidu.com/v2/api/?getapi', params=get_data, headers=headers,verify=False)
...
def get_rsa_key(token, gid, callback):
    ...
    resp = session.get(url='https://passport.baidu.com/v2/getpublickey', headers=headers, params=get_data,verify=False)
    if resp.status_code == 200 and callback in resp.text:
        data = json.loads(re.search(r'.*?\((.*)\)', resp.text).group(1).replace("'", '"'))
...
if __name__ == '__main__':
    ...
    home_page = session.get('http://pan.baidu.com/disk/home', headers=headers,verify=False).text
    print(home_page)
